...
class MopPluginResolver implements PluginResolver {
    ...
    private Predicate<Artifact> getArtifactFilter() {
        if (ARTIFACT_FILTER == null) {
            ...
            Set<Artifact> deps;
            try {
                deps = getMopRepository().resolveArtifacts(new ArtifactId[] { ArtifactId.parse(CLOUDLAUNCH_GROUP_ID + ":" + CLOUDLAUNCH_ARTIFACT_ID, defaultPluginVersion, MOP.DEFAULT_TYPE) });
            } catch (Exception e) {
